...
class TVODEEvaluator:
    ...
    def load_kernel_context(self, kernel_path: str):
        ...
        for device in kernel.get('micro_devices', []):
            ...
            # Only store first instance of each device (usually the primary definition)
            # Later instances are section-specific variants we can ignore
            if device_name_lower in self.kernel_devices:
                continue  # Skip duplicate device entries
            
            # Store with lowercase key
            # FIX: Use pedagogical_function (has actual description) instead of function (just codes like "Me")
            self.kernel_devices[device_name_lower] = {
                'definition': device.get('definition', ''),
                'function': device.get('pedagogical_function', device.get('function', '')),  # Prefer pedagogical_function
                ...
                'examples': device.get('examples', [])  # Store examples for context
            }
        ...
        # Diagnostic: show first device to verify pedagogical_function is loading
        if self.kernel_devices:
            first_device = list(self.kernel_devices.keys())[0]
            first_data = self.kernel_devices[first_device]
            func_preview = first_data.get('function', '')[:60]
            print(f"  Sample: '{first_device}' → function: '{func_preview}...'")
